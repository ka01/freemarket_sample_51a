$(function(){
  ...
  $(document).on('change', 'input[type= "file"].sell-upload-drop-box__file',function(e) {
    ...
    var new_image = $(`<input multiple="multiple" name="item_images[image_url][]" class="sell-upload-drop-box__file" data-image= ${images.length}> type="file" id="sell-upload-drop-box__file"`);
  ...
  $(document).on('click', '.delete', function() {
    ...
    $('input[type= "file"].sell-upload-drop-box__file:first').attr({
    ...
    $.each(inputs, function(index, input) {
      ...
      $('input[type= "file"].sell-upload-drop-box__file:first').after(input)
    })
    if (images.length >= 5) {
      dropzone2.css({
        'display': 'block'
    ...
    if(images.length == 3) {
      dropzone.find('i').replaceWith('<p>ココをクリックしなさい</p>')
    }
  })
});